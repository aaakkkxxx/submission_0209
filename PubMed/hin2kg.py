...
directory = "PubMed"
link_info = [i.strip().split()[:3] for i in open(directory+"/link.dat").readlines()]
node_info = [i.strip().split()[:3] for i in open(directory+"/node.dat").readlines()]
labelled_D = [i.strip().split()[:3] for i in open(directory+"/label.dat").readlines()]
name = {0:"GENE", 1:"DISEASE", 2:"CHEMICAL", 3:"SPECIES"}


d_list = [i[0] for i in labelled_D]

g_list, d_list_new, c_list, s_list = [], [], [], []
for i in link_info:
    if i[1] in d_list: 
        if int(i[2]) == 1:
            g_list.append(i[0])
...
ent_list = []
ent_list.extend(g_list)
ent_list.extend(d_list)
ent_list.extend(c_list)
ent_list.extend(s_list)
ent2id = dict()
for ent in ent_list:
...
for i in ent_list:
    if i in g_list:
        new_node_info.append((ent2id[i], "G", 0))
    elif i in d_list:
        new_node_info.append((ent2id[i], "D", 1))
    elif i in c_list:
        new_node_info.append((ent2id[i], "C", 2))
    elif i in s_list:
        new_node_info.append((ent2id[i], "S", 3))
        

new_link_info = []
for i in link_info:
    if i[0] in ent_list and i[1] in ent_list:
        new_link_info.append((ent2id[i[0]], ent2id[i[1]]))
